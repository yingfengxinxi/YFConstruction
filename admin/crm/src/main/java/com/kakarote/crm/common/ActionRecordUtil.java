...
public class ActionRecordUtil {
    ...
    static {
        ...
        //费用
        propertiesMap.put(CrmEnum.EXPENSE.getType(),
                Dict.create().set("expenseNum", "费用编号").set("customerId", "客户姓名")
                        .set("contractId", "合同编号").set("expenseTime", "费用日期")
                        .set("money", "费用金额").set("description", "费用描述").set("expenseType", "费用类型")
                        .set("expenseTime", "费用日期").set("hasInvoice", "是否有发票").set("invoiceNums", "发票号码")
                        .set("remarks", "备注")
        );
        //费用审批
        propertiesMap.put(CrmEnum.EXPENSE_EXAMINE.getType(),
                Dict.create().set("content", "报销说明").set("money", "费用总额")
        );
    }
