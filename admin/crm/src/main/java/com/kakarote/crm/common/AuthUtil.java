...
public class AuthUtil {
    ...
    public static String getCrmAuthSql(CrmEnum crmEnum, String alias, Integer readOnly) {
        ...
        if (longs != null && longs.size() > 0) {
            ...
            }else if(crmEnum.equals(CrmEnum.EXPENSE)){//TODO 费用权限
                conditions.append(" and ({alias}create_user_id in (").append(StrUtil.join(",", longs)).append(")");
            } else {
    ...
    public static List<Long> getUserIdByAuth(String realm) {
        List<Long> authUserIdList = new ArrayList<>();
        AdminService adminService = ApplicationContextHolder.getBean(AdminService.class);
        List<Long> allUserIdList = adminService.queryUserList().getData();
        if (UserUtil.isAdmin()) {
            ...
            Integer dataType = adminService.queryDataTypeByRealm(UserUtil.getUserId(), realm).getData();
            authUserIdList = getAuthUserIdByDataType(authUserIdList, allUserIdList, dataType);
        }
        return authUserIdList;
    }
    ...
    public static Map<String, Integer> getDataTypeByUserId() {
        ...
        if (CollectionUtil.isEmpty(map)) {
            ...
            if (!UserUtil.isAdmin()) {
                ...
                Integer leadsDataType = adminService.queryDataTypeByRealm(userId, "leads").getData();
                map.put("leads", leadsDataType);
                Integer customerMenuId = 29;
                Integer customerDataType = adminService.queryDataTypeByRealm(userId, "customer").getData();
                map.put("customer", customerDataType);
                Integer contactsMenuId = 43;
                Integer contactsDataType = adminService.queryDataTypeByRealm(userId, "contacts").getData();
                map.put("contacts", contactsDataType);
                Integer businessMenuId = 49;
                Integer businessDataType = adminService.queryDataTypeByRealm(userId, "business").getData();
                map.put("business", businessDataType);
                Integer contractMenuId = 56;
                Integer contractDataType = adminService.queryDataTypeByRealm(userId, "contract").getData();
                map.put("contract", contractDataType);
                Integer receivablesMenuId = 63;
                Integer receivablesDataType = adminService.queryDataTypeByRealm(userId, "receivables").getData();
                map.put("receivables", receivablesDataType);
                Integer productMenuId = 68;
                Integer productDataType = adminService.queryDataTypeByRealm(userId, "product").getData();
