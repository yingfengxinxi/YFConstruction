...
import com.kakarote.build.ai.mapper.AiDregCarMapper;
...
public class AiDregCarServiceImpl extends BaseServiceImpl<AiDregCarMapper, AiDregCar> implements IAiDregCarService {
    ...
    @Override
    public BasePage<AiDregCar> selectByPage(AiDregCarBO aiDregCarBO) {
        aiDregCarBO.setOrdersBd("c.transfer_time",false);
        BasePage<AiDregCar> list = getBaseMapper().selectByPage(aiDregCarBO.parse(), aiDregCarBO);
        return list;
    }
        ...
        try {
            if(list != null && list.size()>0){
                ...
                for(Map data:list){
                    ...
                    if("in".equals(data.get("alarmStatus"))){
                        aiDregCar.setDirection("1");
                    }
                    if("out".equals(data.get("alarmStatus"))){
