...
import com.kakarote.build.equipment.mapper.EquipmentElectricityRecordMapper;
 ...
 */
@Service
public class EquipmentElectricityRecordServiceImpl extends BaseServiceImpl<EquipmentElectricityRecordMapper, EquipmentElectricityRecord> implements IEquipmentElectricityRecordService {

    @Autowired
    private IEquipmentAssetsService assetsService;

    @Override
    public Map<String, Object> selectTableList(EquipmentElectricityRecordBO electricityRecordBO) {
        if (electricityRecordBO.getDateType() == null) {
        ...
        if (electricityRecordBO.getDateType().equals("1")) {
            // 参数未传查询时间，则默认查询当年数据
            if (electricityRecordBO.getDate() == null) {
                Date currentTime = new Date();
                electricityRecordBO.setDate(currentTime);
            }
            cols = getCols(electricityRecordBO.getDate(), electricityRecordBO.getDateType());
            // 设置时间格式化字符串
            electricityRecordBO.setDateFormat("%Y-%m");

        } else if (electricityRecordBO.getDateType().equals("2")) {
            // 参数未传查询时间，则默认查询当月数据
            if (electricityRecordBO.getDate() == null) {
                Date currentTime = new Date();
                electricityRecordBO.setDate(currentTime);
            ...
            cols = getCols(electricityRecordBO.getDate(), electricityRecordBO.getDateType());
            // 设置时间格式化字符串
            electricityRecordBO.setDateFormat("%Y-%m-%d");
        } else if (electricityRecordBO.getDateType().equals("3")) {

            if (electricityRecordBO.getStartDate() == null) {
                throw new CrmException(500, "开始时间不能为空");
            }
            if (electricityRecordBO.getEndDate() == null) {
                throw new CrmException(500, "结束时间不能为空");
            }

            String pattern = "yyyy-MM-dd";
            try {
                cols = com.kakarote.build.utils.DateUtil.getDateStrList(DateUtil.format(electricityRecordBO.getStartDate(), pattern), DateUtil.format(electricityRecordBO.getEndDate(), pattern), pattern);
        ...
        electricityRecordBO.setCols(cols);


        List<LinkedHashMap<String, Object>> list = getBaseMapper().selectTableList(electricityRecordBO);
    ...
    public JSONObject excelImport(MultipartFile file) {
        ...
        String message = "";
        Integer errSize = 0;
        List<EquipmentElectricityRecord> recordList = new ArrayList<>();

        for (int i = 0; i < list.size(); i++) {
            boolean isError = false;
            EquipmentElectricityRecord record = new EquipmentElectricityRecord();
            ...
            // 用电量
            String electricQuantity = String.valueOf(map.get("electricQuantity"));
            if (electricQuantity == null || electricQuantity.equals("")) {
                message += "第" + (i + 4) + "行用电量不能为空,请检查数据；";
                isError = true;
            }
            ...
            if (!isError) {
                ...
                record.setElectricQuantity(electricQuantity);
    ...
    @Override
    public Map<String, Object> selectProTableList(EquipmentElectricityRecordBO electricityRecordBO) {
        if (electricityRecordBO.getDateType() == null) {
            throw new CrmException(500, "报表类型不能为空");
        }
        // 日期时间列表
        ...
        List<LinkedHashMap<String, Object>> list = getBaseMapper().selectProTableList(electricityRecordBO);
