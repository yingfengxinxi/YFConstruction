...
public class OaExamineCategoryController {
    ...
    @ApiOperation("查询对应审批类型信息")
    @PostMapping("/selectExamineCategoryByType/{type}")
    public Result<OaExamineCategory> selectExamineCategoryByType(@PathVariable("type") Integer type) {
        OaExamineCategory examineCategoryList = examineCategoryService.selectExamineCategoryByType(type);
        return Result.ok(examineCategoryList);
    }
