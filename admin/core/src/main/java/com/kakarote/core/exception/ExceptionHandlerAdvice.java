...
public class ExceptionHandlerAdvice {
    ...
    public Result myBatisException(MyBatisSystemException ex) {
        //TODO MyBatis 异常
        log.error("MyBatis异常处理", ex.getCause().getCause());
        //自定义扩展业务异常类型
        if(ex.getCause().getCause() instanceof CrmException){
            CrmException exception = (CrmException)ex.getCause().getCause();
