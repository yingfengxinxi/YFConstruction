...
public class ProjectSqlParser extends AbstractJsqlParser {
    ...
    public boolean doFilter(MetaObject metaObject, String sql) {
        ...
        //System.out.println(mapperName + "." + actionName);
        //mapper校验
        for (String mapperMatch : DataIsolationConfig.ProjectIgnoreMappers) {
            if (WildcardMatchUtil.isMatch(mapperName, mapperMatch)) {
                return false;
            }
        }
        //action 校验
        for (String actionMatch : DataIsolationConfig.ProjectIgnoreMapperActions) {
            if (WildcardMatchUtil.isMatch(mapperName + "." + actionName, actionMatch)) {
                return false;
            }
        }
        //todo 2022.03.07 识别用户属性 V1.0 未登录不进行过滤（暂定）
        UserInfo userInfo = UserUtil.getUser();
        if (userInfo == null || userInfo.getUserId() == null) {//未登录
            return false;
        }
        //2022.05.06 校验是否临时过滤
    ...
    public void processInsert(Insert insert) {
        if (projectHandler.doTableFilter(insert.getTable().getName(), null)) {
            // 过滤退出执行
            return;
        }
        //新增和修改需要判断columns是否包含proId
        List<Column> list = insert.getColumns();
        for (Column column : list) {
            if (column.getColumnName().equals(projectHandler.getProjectIdColumn())) {
                return;
            }
        }
        insert.getColumns().add(new Column(projectHandler.getProjectIdColumn()));
        if (insert.getSelect() != null) {
            ...
            if (itemsList instanceof MultiExpressionList) {
                ((MultiExpressionList) itemsList).getExprList().forEach(el -> el.getExpressions().add(projectHandler.getProjectId(false)));
            } else {
                ((ExpressionList) insert.getItemsList()).getExpressions().add(projectHandler.getProjectId(false));
            }
        } else {
    ...
    public void processUpdate(Update update) {
        ...
        if (projectHandler.doTableFilter(table.getName(), null)) {
            // 过滤退出执行
            return;
        }
    ...
    public void processDelete(Delete delete) {
        if (projectHandler.doTableFilter(delete.getTable().getName(), null)) {
            // 过滤退出执行
            return;
        }
    ...
    protected BinaryExpression andExpression(Table table, Expression where) {
        ...
        equalsTo.setRightExpression(projectHandler.getProjectId(true));
        if (null != where) {
    ...
    protected void processPlainSelect(PlainSelect plainSelect, boolean addColumn) {
        ...
        if (fromItem instanceof Table) {
            ...
            if (!projectHandler.doTableFilter(fromTable.getName(), plainSelect.getWhere())) {
                //#1186 github
                plainSelect.setWhere(builderExpression(plainSelect.getWhere(), fromTable));
                if (addColumn) {
                    plainSelect.getSelectItems().add(new SelectExpressionItem(
                            new Column(projectHandler.getProjectIdColumn())));
                }
            }
        } else {
    ...
    protected void processJoin(Join join) {
        if (join.getRightItem() instanceof Table) {
            ...
            if (this.projectHandler.doTableFilter(fromTable.getName(), join.getOnExpression())) {
                // 过滤退出执行
                return;
            }
    ...
    public Expression builderExpression(Expression currentExpression, Table table) {
        final Expression tenantExpression = projectHandler.getProjectId(false);
    ...
    protected Column getAliasColumn(Table table) {
        ...
        column.append(projectHandler.getProjectIdColumn());
