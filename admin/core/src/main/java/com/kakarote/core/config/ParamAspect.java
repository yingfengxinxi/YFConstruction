...
public class ParamAspect implements Ordered {
    ...
    @Around("(execution(* com.*.*.controller..*(..))||execution(* com.*.*.*.controller..*(..))) " +
            "&& execution(@(org.springframework.web.bind.annotation.*Mapping) * *(..))  " +
            "&& !execution(@(com.kakarote.core.common.ParamAspect) * *(..))")
