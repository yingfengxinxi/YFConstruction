...
//            throw new CrmException(WorkCodeEnum.WORK_CREATE_NAME_NULL_ERROR);
    ...
    public List<WorkTaskTemplateClassVO> queryTaskByWorkId(WorkTaskTemplateBO workTaskTemplateBO) {
        //2020-11-30 存入租户ID
        workTaskTemplateBO.setTenantId(UserUtil.getUser().getTenantId());
        ...
        finalClassList.forEach(workClass -> {
            List<TaskInfoVO> taskList = getBaseMapper().queryTaskByClassId(workTaskTemplateBO, workClass.getClassId(), UserUtil.getUserId());
    ...
    public List<WorkTaskTemplateUserVO> queryOwnerTaskListByWorkId(WorkTaskTemplateBO workTaskTemplateBO) {
        //2020-11-30 存入租户ID
        workTaskTemplateBO.setTenantId(UserUtil.getUser().getTenantId());
        List<TaskInfoVO> taskList = getBaseMapper().queryTaskByClassId(workTaskTemplateBO, null, UserUtil.getUserId());
        if (taskList.size() == 0) {
    ...
    public WorkStatsVO workStatistics(String workType) {
        ...
        if ("all".equals(workType)) {
            ...
            if (workAuthUtil.isWorkAdmin()) {
                taskStatistics = getBaseMapper().workStatistics(null, null, null, null,UserUtil.getUser().getTenantId());
                memberTaskStatistics = getBaseMapper().queryAllTaskManUser();
                memberTaskStatistics.forEach(userStatsVO -> {
                    WorkTaskStatsVO first = getBaseMapper().workStatistics(null, null, null, userStatsVO.getUserId(),UserUtil.getUser().getTenantId());
                ...
                if (workIds.size() == 0) {
                    ...
                    taskStatistics = getBaseMapper().workStatistics(null, workIds, null, null,UserUtil.getUser().getTenantId());
                    ...
                    ;
                    memberTaskStatistics.forEach(userStatsVO -> {
                        WorkTaskStatsVO first = getBaseMapper().workStatistics(null, workIds, null, userStatsVO.getUserId(),UserUtil.getUser().getTenantId());
                        BeanUtil.copyProperties(first, userStatsVO);
                    });
                }
            }
        ...
        WorkTaskStatsVO taskStatistics = getBaseMapper().workStatistics(workId, null, null, null,UserUtil.getUser().getTenantId());
    ...
    private List<WorkUserStatsVO> memberTaskStatistics(Integer workId) {
        ...
        if (work.getIsOpen() == 1) {
            ...
            list.forEach(userStatsVO -> {
                WorkTaskStatsVO first = getBaseMapper().workStatistics(workId, null, null, userStatsVO.getUserId(),UserUtil.getUser().getTenantId());
                BeanUtil.copyProperties(first, userStatsVO);
            });
        } else {
            ...
            for (String userId : ownerUserIds.split(",")) {
                ...
                WorkTaskStatsVO first = getBaseMapper().workStatistics(workId, null, Long.parseLong(userId), null,UserUtil.getUser().getTenantId());
    ...
    @Override
    public BasePage<Map<String, Object>> queryProject(Integer customerId, PageEntity pageEntity) {
        BasePage<Map<String, Object>> basePage = pageEntity.parse();
//        String conditions = AuthUtil.getCrmAuthSql(CrmEnum.CONTRACT, "a", 1);
        BasePage<Map<String, Object>> page = getBaseMapper().queryProject(basePage, customerId, UserUtil.getUserId().intValue());
        return page;
    }
}
