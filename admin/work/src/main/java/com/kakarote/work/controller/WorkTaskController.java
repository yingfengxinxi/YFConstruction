...
import com.kakarote.work.entity.VO.EventTaskVO;
...
public class WorkTaskController {
    ...
    @GetMapping("/getTaskInfo/{tenantId}/{taskId}")
    @ApiOperation("查询任务详情")
    public JSONObject getTaskInfo(@PathVariable Long tenantId, @PathVariable Integer taskId){
        WorkTask workTask = workTaskService.lambdaQuery()
                .eq(WorkTask::getTenantId,tenantId)
                .eq(WorkTask::getTaskId,taskId).one();
        String jsonString = JSONObject.toJSONString(workTask);
    ...
    public Result<List<EventTaskVO>> eventTask(@RequestBody QueryEventTaskBO eventTaskBO) {
        List<EventTaskVO> eventTaskVOList = workTaskService.eventTask(eventTaskBO);
