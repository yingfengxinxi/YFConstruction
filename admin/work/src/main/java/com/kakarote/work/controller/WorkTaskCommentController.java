...
public class WorkTaskCommentController {
    ...
    @PostMapping("/dtalkDeleteComment")
    @ApiOperation("删除评论")
    public Result dtalkDeleteComment(@RequestBody Map<String, Object> params) {
    ...
    @PostMapping("/getComment/{tenantId}/{commentId}")
    @ApiOperation("获取评论详情")
    public JSONObject getComment(@PathVariable("tenantId") Long tenantId, @PathVariable("commentId") Integer commentId) {
        WorkTaskComment comment = workTaskCommentService.lambdaQuery()
                .eq(WorkTaskComment::getTenantId, tenantId)
                .eq(WorkTaskComment::getCommentId, commentId).one();
