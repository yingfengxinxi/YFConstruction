...
public class WorkTaskServiceImpl extends BaseServiceImpl<WorkTaskMapper, WorkTask> implements IWorkTaskService {
    ...
    public void taskListTransfer(List<TaskInfoVO> taskList) {
        ...
        taskList.forEach(taskInfo -> {
            ...
            //20201216:追加里程碑查询
            List<WorkTaskMilestone> mList = workTaskMilestoneService.query()
                    .eq("task_id", taskId)
                    .eq("status", 1)
                    .eq("user_id", UserUtil.getUserId()).list();
            taskInfo.setIsMilestone(mList.size() > 0 ? 1 : 0);
        });
    }
    ...
    public void saveWorkTask(WorkTask task) {
        ...
        if (isSave) {
            ...
            //TODO 钉钉消息推送
            if (task.getMainUserId() != null && !task.getMainUserId().equals(user.getUserId())) {
                //messageService.proSend(user.getTenantId(), task.getTaskId(), UserUtil.getUserId());
            }
        }
    ...
    public void setWorkTaskMainUser(WorkTaskUserBO workTaskUserBO) {
        ...
        if (mainUserId == null) {
            ...
            //TODO 消息提醒
            //messageService.proSend(UserUtil.getUser().getTenantId(), taskId, UserUtil.getUserId());
        }
        workTaskLogService.saveWorkTaskLog(workTaskLog);
    }
    ...
    public void setWorkTaskOwnerUser(WorkTaskOwnerUserBO workTaskOwnerUserBO) {
        ...
        //TODO 消息提醒
        //messageService.proSend(UserUtil.getUser().getTenantId(), workTask.getTaskId(), UserUtil.getUserId());
    ...
    public TaskDetailVO queryTrashTaskInfo(Integer taskId) {
        ...
        List<WorkTaskMilestone> mList = workTaskMilestoneService.query()
                ...
                .eq("status", 1)
                .eq("user_id", UserUtil.getUserId()).list();
        taskDetailVO.setIsMilestone(mList.size() > 0 ? 1 : 0);
        return taskDetailVO;
    }
...
//        Db.delete("delete from wk_crm_activity where type = 2 and activity_type = 11 and activity_type_id = ?",taskId);
    ...
    public List<EventTaskVO> eventTask(QueryEventTaskBO eventTaskBO) {
        ...
        return getBaseMapper().queryEventTask(eventTaskBO);
    }
