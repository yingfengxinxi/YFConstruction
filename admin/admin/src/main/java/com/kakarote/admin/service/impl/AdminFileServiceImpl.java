...
public class AdminFileServiceImpl extends BaseServiceImpl<AdminFileMapper, AdminFile> implements IAdminFileService {
    ...
    public List<FileEntity> queryFileList(List<String> batchIdList) {
        ...
        List<AdminFile> list = lambdaQuery().in(AdminFile::getBatchId, batchIdList).orderBy(true,true,AdminFile::getCreateTime).list();
        List<FileEntity> entityList = new ArrayList<>();
        parseFileEntity(list, entityList);
        return entityList;
    }
    ...
    @Override
    public FileEntity queryOneByBatchIdAndPath(Map map) {
        String batchId = (String)map.get("batchId");
        String path = (String)map.get("path");

        FileEntity entity = new FileEntity();
        AdminFile adminFile = lambdaQuery()
                .eq(AdminFile::getBatchId, batchId)
                .eq(AdminFile::getPath, path)
                .last("limit 1").one();
        if (adminFile != null) {
            entity.setIsPublic(adminFile.getIsPublic().toString());
            entity.setPath(adminFile.getPath());
    ...
    public void down(HttpServletResponse response, Long fileId) {
        AdminFile adminFile = getBaseMapper().getByIdWx(fileId);
        if (adminFile != null) {
    ...
    @Override
    public List<Map> queryFileListByBatchId(String batchId) {
        QueryWrapper queryWrapper = new QueryWrapper();
        queryWrapper.eq("batch_Id",batchId);
        queryWrapper.select("path as pathUrl","file_id");
     ...
     */
    @Override
    public UploadEntity uploadWx(MultipartFile file, String batchId, String type, String isPublic,String tenantId) throws IOException {
        if (StrUtil.isEmpty(batchId)) {
            batchId = IdUtil.simpleUUID();
        }
        UploadEntity entity = new UploadEntity(MybatisPlusConfig.getNextId() + "", file.getOriginalFilename(), file.getSize(), batchId,isPublic);
        ...
        Integer sz=1;
        adminFile.setCreateUserId(sz.longValue());
        adminFile.setTenantId(tenantId);
        getBaseMapper().add(adminFile);
        return entity;
    }
     ...
     */
    @Override
    public List<FileEntity> queryFileListWx(List<String> batchIdList) {
        String batchId="";
        if (batchIdList.size() == 0) {
            return new ArrayList<>();
        }else if(batchIdList.size()>0){
            batchId=batchIdList.get(0);
        }
        List<AdminFile> list =  getBaseMapper().selectListWx(batchId);
        List<FileEntity> entityList = new ArrayList<>();
        parseFileEntityWx(list, entityList);
        return entityList;
    }


    private void parseFileEntityWx(List<AdminFile> list, List<FileEntity> entityList) {
        list.forEach(adminFile -> {
            FileEntity entity = new FileEntity();
            entity.setIsPublic(adminFile.getIsPublic().toString());
            entity.setPath(adminFile.getPath());
            ...
            entity.setName(adminFile.getName());
            entity.setCreateTime(adminFile.getCreateTime());
            entity.setCreateUserName("wx");
            entity.setCreateUserId(adminFile.getCreateUserId());
            entity.setSize(adminFile.getSize());
            entity.setFileType(adminFile.getFileType());
            entity.setBatchId(adminFile.getBatchId());
            entity.setUrl( "/adminFile/downWx?fileId=" + adminFile.getFileId().toString());
            entityList.add(entity);
        });
    }
     ...
     */
    @Override
    public void downWx(HttpServletResponse response, Long fileId) {
        AdminFile adminFile = getBaseMapper().getByIdWx(fileId);
        if (adminFile != null) {
            if (Objects.equals(1, adminFile.getType())) {
                try {
    ...
    @Override
    public void add(AdminFile adminFile) {
        save(adminFile);
    }
}
