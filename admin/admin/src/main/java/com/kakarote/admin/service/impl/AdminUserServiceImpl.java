...
public class AdminUserServiceImpl extends BaseServiceImpl<AdminUserMapper, AdminUser> implements IAdminUserService {
    ...
    @Override
    public List<Map<String, Object>> findByNameConfig(String username, String configId) {
        Integer systemStatus = querySystemStatus();
        if (systemStatus == 0) {
            throw new CrmException(SystemCodeEnum.SYSTEM_USER_NOT_REGISTER_ERROR);
        }
        return getBaseMapper().findByNameConfig(username, configId);
    }
    ...
    public BasePage<AdminUserVO> queryUserList(AdminUserBO adminUserBO) {
        ...
        if (adminUserBO.getDeptId() != null) {
            ...
            if (roleList.stream().filter(w -> w.getDataType() == 5).findAny().isPresent()) {
                ...
                AdminDept adminDept = adminDeptService.getById(UserUtil.getUser().getDeptId());
                List<Integer> list = adminDeptService.queryChildDept(adminDept.getPid());
                list.add(adminUserBO.getDeptId());
                adminUserBO.setDeptIdList(list);
            } else if (roleList.stream().filter(w -> w.getDataType() == 2 || w.getDataType() == 1).findAny().isPresent()) {
                List<Integer> list = adminDeptService.queryChildDept(UserUtil.getUser().getDeptId());
                list.add(adminUserBO.getDeptId());
                adminUserBO.setDeptIdList(list);
            }
        }

        if (adminUserBO.getRoleType() != null) {
            List<AdminRole> recordList = adminRoleService.lambdaQuery().select(AdminRole::getRoleId).eq(AdminRole::getRoleType, adminUserBO.getRoleType()).list();
            if (recordList.size() > 0) {
                adminUserBO.setRoleId(recordList.get(0).getRoleId());
            }
        }
        BasePage<AdminUserVO> basePage = getBaseMapper().queryUserList(adminUserBO.parse(), adminUserBO, UserUtil.getUser().getTenantId().intValue());
...
//        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long addUser(AdminUserVO adminUser) {
...
//        }
        ...
        //20210105:手机号为登录名,租户内唯一  20220316:若参数内包含tenantId 则参数数据优先
        Integer tenantId = adminUser.getTenantId();
        if (tenantId == null) {
            tenantId = Integer.valueOf(UserUtil.getUser().getTenantId().toString());
        }
        Integer count = query().eq("username", adminUser.getUsername()).eq("tenant_id", tenantId).count();
        if (count > 0) {
    ...
    public JSONObject excelImport(MultipartFile file) {
        ...
        ExcelUtil.readBySax(filePath, 0, (int sheetIndex, int rowIndex, List<Object> rowList) -> {
            if (rowIndex > 1) {
                ...
                adminUser.setStatus(1);//TODO 暂时取消激活机制，添加后直接激活
...
//                }
    ...
    public BasePage<UserBookVO> queryListName(UserBookBO userBookBO) {
        ...
        userBookBO.setTenantId(UserUtil.getUser().getTenantId());
    ...
    public String getNameByUserId(Long userId) {
        ...
        AdminUser adminUser = query().select("realname").eq("user_id", userId).ne("tenant_id", 0).one();
        if (adminUser == null) {
            return "";
        }
    ...
    public void delUser(Long userId) {
        ...
        wrapper.ne("tenant_id", 1);//非顶级租户
        wrapper.eq("user_id", userId);
        remove(wrapper);
    }

    @Override
    public List<Map<String, Object>> findTenantList(String username) {
        Integer systemStatus = querySystemStatus();
        if (systemStatus == 0) {
            throw new CrmException(SystemCodeEnum.SYSTEM_USER_NOT_REGISTER_ERROR);
        }
        return getBaseMapper().findTenantList(username);
    }

    @Override
    public void newUpdateUser(AdminUser adminUser) {
        if (!UserUtil.getUser().getUsername().equals(adminUser.getUsername())) {
            throw new CrmException(AdminCodeEnum.ADMIN_USERNAME_EDIT_ERROR);
        }
        adminUser.setUserId(UserUtil.getUserId());
