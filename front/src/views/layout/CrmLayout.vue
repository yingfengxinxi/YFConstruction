...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['basePath','applicationId', 'crm', 'routers']),
      // 快捷添加
      quickAddList() {
        var addItems = []
        if (this.crm.expense && this.crm.expense.examines) {
          addItems.push({
            icon: 'expense',
            index: 'expense-examines',
            label: '报销'
          })
        }
        if (this.crm.expense && this.crm.expense.save) {
          addItems.push({
            icon: 'expense',
            index: 'expense',
            label: '费用'
    ...
    mounted() {
      var crmRouters = this.routers[this.applicationId];
      this.routerItems = crmRouters.children;
      //自动选择第一个
