			...
			<div class="sheb_list_box">
				...
				<div class="list_box">
					...
					<div class="list_con">
						...
						<div class="zwsj" v-show="equipmentlist.length==0">
							暂无数据
						</div>
					</div>
				</div>
			</div>
			<div class="sheb_echarts" ref='sbgd'>
				<div class="subject"><img src="../../../../../static/img/Symbol.png">{{assetName}}沉降</div>
				<div id="shebInfo" v-show="cjlist.length>0">

				</div>
				<div class="zwsj" v-if="cjlist.length==0">
							暂无数据
						</div>
			</div>
		</div>
		<div class="foun_center">
			<div class="swiper_box">
				...
				<div class="swiper_btn">
					...
					<div v-if="swiperList.length==0" class="zwsj">
							暂无数据
						</div>
				</div>
			</div>
			...
			<div v-if="swiperList.length>0" id="container"></div>
			<div v-if="swiperList.length==0" class="zwsj">
							暂无数据
						</div>
		</div>
...
<script>
	...
	export default {
		...
		mounted() {
			document.getElementById('shebInfo').style.height=this.$refs.sbgd.offsetHeight-40+'px'
			this.remsize()
		},
		methods: {
						//C4D------------------------------
			...
			init(el, obj,objposition) {
				...
				//加载图片
				// var geometry=new THREE.PlaneGeometry(19200,10800);
				// var texture=new THREE.TextureLoader().load(window.location.pathname+'jz.jpg');
				// var material=new THREE.MeshLambertMaterial({map:texture,side:THREE.DoubleSide,transparent:true,});
				// var mesh=new THREE.Mesh(geometry,material);that.scene.add(mesh);mesh.rotateX(-Math.PI/2);mesh.position.y=-0.01
				...
				that.camera.position.set(150, 500, 300)
				...
				// this.orbitControls.autoRotate = false; //自动旋转开关
				...
				for(let i=0;i<objposition.length;i++){
					if(objposition[i].c4dPosition){
						...
						moonDiv['div_'+i].innerHTML = `<div style='position:absolute;width:1px;height:70px;background:#5B96FA;top:-70px'></div>
						<div style='width:10px;height:10px;background:rgb(98, 222, 135);border-radius:50%;position:absolute;top:-5px;left:-5px;'></div>
						<div style='position:absolute;width:150px;top:-130px;border:1px solid #5B96FA;left:-75px;background:rgba(0,0,0,0.5);border-radius:10px;padding:10px';>
...
<style scoped lang="scss">
	...
	.jk_box{
		...
		height: 60%;
		margin-top: 0.36rem;
		background: url(./img/jk_bg.png) no-repeat;
		background-size: 100% 100%;
	}
	.foun_right{
		width: 4.58rem;
