<template>
  <div class="dom">
    <div class="dom_1">
      ...
      <img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'     v-for="item in elevators"
            :key="item.assetId"
            class="dtimg"
            :style="{top:item.planeCoordinatePcX+'px',left:item.planeCoordinatePcY+'px'}"
            @click="goStateList2(item.assetId,item.assetName)"
            src="../../../../../static/img/towerCraneMonitor/dtdgimg.png" />
    </div>
    <div class="dom_2">
      <div class="dom_2_t">
        <div class="subject"><img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    src="../../../../../static/img/Symbol.png">今日吊钩工作情况</div>
        <div class="gzxq_dom">
          <div class="gzxq_doms" >
      ...
      <div class="dom_2_b">
        ...
        <div class="table_dom">
          <table cellpadding="0" cellspacing="0" class="my_table">
            <tr v-for="item in elevators.slice(0,8)">
...
<script>
  ...
  import { getHookList,getAlertList} from './api/elevator'
  ...
  export default {
    ...
    methods: {
      ...
      init(){
        getHookList().then(res => {
          this.elevators = res.data;
          this.elevators.forEach(function (item) {
            if(item.line == 1){
              taht.line++;
            }
          })
      ...
      goStateList(assetId){
        this.goToUrl('hookBlindVisual', {assetId:assetId})
      },
      goStateList2(assetId,name){
