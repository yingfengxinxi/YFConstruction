	...
	<div id="cloudCenter">
		...
		<div class="center_box">
			<div class="police_list_box">
				...
				<div class="police_list">
					<div class="police_list_title">
						<div style="width: 20%;">报警类型</div>
						<div style="width: 15%;">报警上限</div>
						<div style="width: 15%;">报警下限</div>
						<div style="width: 20%;">超标值</div>
						<div style="width: 15%;">报警时间</div>
						<div style="width: 15%;">处理时间</div>
					</div>
					<div class="prolice_list_conbox">						
						<div class="police_list_con" v-for="item in datalist">
							<div style="width: 20%;">{{item.monitorTypeName ? item.monitorTypeName : '-'}}</div>
							<div style="width: 15%;">{{item.alarmUpper ? item.alarmUpper : '-'}}</div>
							<div style="width: 15%;">{{item.alarmLower ? item.alarmLower : '-'}}</div>
							<div style="width: 20%;">{{item.monitorData}}{{item.unit}}</div>
							<div style="width: 15%;">{{item.createTime}}</div>
							<div style="width: 15%;">{{item.handleTime}}</div>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="airdata_box">
				...
				<div class="dust_tab">
					<div class="dust_tab_con" :class="airdataIndex==1?'dust_active':''" @click="airClick(1)">全部</div>
					<div>
						<el-date-picker
		...
		<div class="right_box">
			...
			<div class="ai_box">
				<div class="more" @click="gotoDregCarAlert">
					更多>>
				</div>
				<div class="model_title">
					<img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    src="../../../../assets/img/projectAnalyse/model_title.png"/>
				...
				<div class="ai_content">
					<div class="ai_img_div" v-for="(item,index) in dregCarAlertList">	
						<div class="ai_img_content">
							<img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    :src="item.alertImg"/>
							<div class="ai_info">
								<img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    src="../../../../assets/img/projectAnalyse/form.png"/>
								<div class="ai_alarm_type">
									{{item.dictLabel}}
								</div>
								<div class="ai_catch_time">
									抓拍时间:{{item.alertTime}}
								</div>
							</div>
						</div>
...
<script>
	...
	import {selectAlertNumber, selectAlertListNew, selectTypeProportionByParent, selectAlertTrend,
		selectRaiseDustAlert, selectProjectAlertNumber, selectNoiseAlert, selectDregCarAlert } from '../api/alertDataAnalyse'
	...
	export default {
		...
		methods: {
			...
			initTypeData() {
				selectTypeProportionByParent().then(res => {		    
			...
			getDregCarAlertList() {
				...
				selectDregCarAlert(params).then(res => {
					this.dregCarAlertList = res.data.list
				})
			},
			goenvAlert(){
