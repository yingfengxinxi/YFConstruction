...
import { saveOrUpdate, getById, selectFBSInfo, selectInfoByMapNoPage, selectWorkTypeInfo } from "./safetymember";
  ...
  computed: {
    title () {
      if (this.action.type === 'showDetials') {
        ...
        return this.action.type === 'update' ? '编辑安全从业人员证件' : '添加安全从业人员证件'
      }
    ...
    peopleChange (date) {
      ...
      Object.keys(datas).some((key) => {
        ...
        if (a == ('' + date)) {
          ...
          //this.$set(this.form, 'workTypeId', datas[key].workTypeId)
          return true
        }
      })
