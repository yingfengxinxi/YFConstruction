	  ...
	  <el-form :rules="formData.rules" :model="formData"  ref="formData" >
		...
		<el-tabs
		  ...
		  class="d-container-bd--left">
		   ...
		   <el-tab-pane v-for="(item,index) in classTypeItemList" :key="index" :label="item.classTypeName" :name="item.classType">
				<el-table
				ref="itemTable"
				v-loading="loading"
				id="crm-table"
				...
				highlight-current-row>
				 ...
				 <el-table-column label="分项描述" align="center" prop="itemDes" width="160" />
				...
				<el-table-column v-if="action.type === 'detail'" label="备注" align="center" prop="scoreDes" width="200" />
				 ...
				 <el-table-column label="附件" align="center">
				   <template  slot-scope="scope">
					...
					<el-button
					  size="mini"
					  type="success"
					  @click="filePageOpen(scope.row.index)">附件窗口</el-button>
				   </template>
				 </el-table-column>
				 </el-table>
		   </el-tab-pane>
...
<script>
  ...
  import { getById, selectLevelList, gradeOrHold} from "./api/eval";
    ...
    computed: {
      title() {
        if(this.action.type === 'detail'){
            ...
            return this.action.type === 'update' ? '打分评级' : '评分申请'
        }

      },
      disabledT() {
        return this.action.type === 'detail' ? true : false
      },
