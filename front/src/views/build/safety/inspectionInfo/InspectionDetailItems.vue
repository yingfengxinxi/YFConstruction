<template>
  <div>
    <create-sections v-for="(item, formIndex) in pageAllItems"
	  ...
	  <el-form :ref="'itemForm'+formIndex" :model="fieldFormAll[formIndex]" :rules="action.type === 'execute' || action.type === 'save' ? rules : rules2" label-position="top" label-width="100px"
	  	class="form">
		<el-row>
		  <el-col :span="8">
		...
		<el-row>
		  <el-col :span="8">
			<el-form-item label="检查类型" prop="place">
			  <el-select
				v-model="fieldFormAll[formIndex].routineCheck"
				placeholder="请选择"
				clearable
		  ...
		  <el-col :span="8">
			<el-form-item label="隐患类别" prop="category">
			  <el-select
				v-model="fieldFormAll[formIndex].category"
				placeholder="请选择"
				clearable
				:disabled="action.isReadonly"
			  >
				<el-option
				  v-for="dict in hazardLevleCategory"
				  :key="dict.dictValue"
				  :label="dict.dictLabel"
				  :value="dict.dictValue"
				/>
		  ...
		  <el-col :span="8">
			<el-form-item label="检查结果" prop="checkResult">
			  <el-select
				v-model="fieldFormAll[formIndex].checkResult"
				placeholder="请选择"
				clearable
				:disabled="action.isReadonly"
			  >
		  ...
		  <el-col :span="8">
			<el-form-item label="整改人电话" prop="rectifyPersonPhone">
			  <el-input v-model="fieldFormAll[formIndex].rectifyPersonPhone" placeholder="请输入整改人电话"></el-input>
			</el-form-item>
		  </el-col>
		  <el-col :span="8">
			<el-form-item label="责任单位" prop="dutyUnit">
			  <el-input v-model="fieldFormAll[formIndex].dutyUnit" placeholder="请输入责任单位"></el-input>
			</el-form-item>
		  </el-col>
		  <el-col :span="8">
			<el-form-item label="责任单位统一社会信用代码" prop="dutyUnitCode">
			  <el-input v-model="fieldFormAll[formIndex].dutyUnitCode" placeholder="请输入责任单位统一社会信用代码"></el-input>
			</el-form-item>
		  </el-col>
		  <el-col :span="8">
			<el-form-item label="劳务班组" prop="serviceClass">
			  <el-select
				v-model="fieldFormAll[formIndex].serviceClass"
				placeholder="请选择"
				clearable
				@change="teamChange($event, formIndex)"
				:disabled="action.isReadonly"
			  >
				<el-option
				  v-for="dict in teamOptionsList"
				  :key="dict.id"
				  :label="dict.teamName"
				  :value="dict.id"
				/>
		  ...
		  <el-col :span="8">
			<el-form-item label="通知人" prop="informPerson">
			  <el-select
				v-model="fieldFormAll[formIndex].informPerson"
				placeholder="请选择"
				clearable
				@change="personChange"
				:disabled="action.isReadonly"
			  >
				<el-option
				  v-for="dict in teamPersonAll[formIndex]"
				  :key="dict.id"
				  :label="dict.name"
				  :value="dict.id"
				/>
        ...
        @change="zgsjbd($event, formIndex)"
		  ...
		  <el-col :span="8">
			<el-form-item label="处罚信息" prop="fineInfo">
			  <el-input v-model="fieldFormAll[formIndex].fineInfo" placeholder="请输入处罚信息"></el-input>
			</el-form-item>
		  </el-col>
		  <!-- <el-col :span="8">
			<el-form-item label="所属公司" prop="place">
			  <el-input v-model="fieldFormAll[formIndex].belongCompany" placeholder="请输入所属公司"></el-input>
			</el-form-item>
		  </el-col> -->
		  <el-col :span="8">
			<el-form-item label="补充信息" prop="supplementInfo">
			  <el-input v-model="fieldFormAll[formIndex].supplementInfo" placeholder="请输入补充信息"></el-input>
			</el-form-item>
		  </el-col>
		</el-row>
...
<script>
  ...
  export default {
    ...
    watch: {
		...
		fieldFormAll(val) {
			this.fieldFormAll.forEach((item, index) => {
				if (item.serviceClass) {
					this.getTeamPersonList(item.serviceClass, index)
				}
			})
		},
	},
    ...
    methods: {
      ...
      deleteItems(index) {
        ...
        this.teamPersonAll.splice(index, 1)
      },
      ...
      this.getDicts('b_hazard_category').then(response => {
        this.hazardLevleCategory = response.data
      })
      this.getDicts('b_inspect_type').then(response => {
        this.inspectType = response.data
      })
	  ...
	  teamChange(value, index) {
		  this.fieldFormAll[index].informPerson = ''
		  this.getTeamPersonList(value, index)
	  },
	  getTeamPersonList(teamId, index) {
		  selectPersonByTeam(teamId).then(res => {
			  this.teamPersonAll[index] = res.data
			  this.$forceUpdate()
		  })
	  },
	  personChange(value) {
      ...
      zgsjbd(e,formIndex){
        ...
        console.log("333333333333333333333="+this.checkDate)
