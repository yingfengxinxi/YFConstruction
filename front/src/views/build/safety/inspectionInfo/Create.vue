<template>
  <xr-create
	...
	<create-sections v-if="action.type != 'save'" title="巡检明细">
	  <inspection-detail-items
	    :action="action"
		:dictData="secondDictData"
		:pageAllItems="pageAllItems"
		:batchIdAll="batchIdAll"
		:fieldFormAll="inspectionDetailList"
...
<script>
  ...
  import {updateInspectionInfo, getById,addInspectionInfo} from "../api/inspectionInfo"
    ...
    computed: {
      title() {
		if (this.action.type == 'save') {
			return '新建检查'
		} else if (this.action.type == 'execute') {
			return '执行检查'
		} else if (this.action.type == 'rectify') {
			return '整改检查'
    ...
    methods: {
      ...
      getDetail() {
        ...
        getById(this.action.id)
          .then(res => {
			...
			this.pageAllItems = pageAllItems
			// 将数据传到子控件
        ...
        this.$refs['form'].validate(valid => {
          if (valid) {
			...
			if (this.action.type == 'save') {
				...
				this.loading = true;
				addInspectionInfo(this.form).then(response => {
				  if (response.code === 0) {
	  ...
	  getPlanList() {
		setTimeout(()=>{
			...
			this.dictDatalisht = res.data
			// console.log(this.dictData.planList,">>>>>>>>>>>000");
		})
		},1000)
