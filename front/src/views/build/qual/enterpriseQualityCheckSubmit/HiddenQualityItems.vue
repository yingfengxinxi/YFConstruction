        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item
              label="真实得分"
              prop="actualScore"
              :error="actualScoreErrorMsg"
            >
              <el-input
                v-model="fieldFormAll[formIndex].actualScore"
                placeholder="请输入真实得分"
                @change="MyChange(formIndex)"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col>
            ...
            >
              <el-input
                type="textarea"
                :rows="1"
                v-model="fieldFormAll[formIndex].situationExplain"
                placeholder="请输入情况说明"
              ></el-input>
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    MyChange(event) {
       this.actualScoreErrorMsg ='';
      // console.log(event);
      let datas = this.fieldFormAll;
      // console.log(datas);
      let actualTempData = datas[event].actualScore; //真实分数
      let fullTempData = datas[event].fullScore; //满分分值
      if (parseInt(actualTempData) > parseInt(fullTempData)) {
        this.actualScoreErrorMsg = "真实得分不能大于满分分值";
        return;
      }
      let TotalScore = 0;
      for (let index = 0; index < datas.length; index++) {
        if (datas[index].hasOwnProperty("actualScore")) {
          if (actualTempData == "" || actualTempData.length == 0) {
            actualTempData = 0;
          }
          TotalScore = TotalScore + parseInt(actualTempData);
        }
      }
      this.$emit("monitor", TotalScore);
    }
  }
};
</script>
