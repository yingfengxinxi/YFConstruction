...
<template>
    <div>
        ...
        <el-popover placement="bottom-start" :width="popoverWidth" trigger="manual" v-model="isShowSelect" @hide="popoverHide">
            <el-tree class="common-tree" :width="width" ref="tree" :data="treeData" :props="obj" :show-checkbox="multiple" :node-key="obj.id" :check-strictly="checkStrictly" :default-expanded-keys="defaultKey" :expand-on-click-node="multiple&&expandClickNode" :check-on-click-node="checkClickNode" :highlight-current="true" @check-change="nodeClick" @node-click="nodeClick"></el-tree>
            <el-select slot="reference" ref="select" :size="size" v-model="returnDataKeys" :multiple="multiple" :clearable="clearable" :collapse-tags="collapseTags" @click.native="selectClick" @remove-tag="removeTag" @clear="clean" class="tree-select">
                <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
            <el-row>
                <el-button v-if="multiple&&treeData.length>0" class="ok" @click="isShowSelect=false" size="mini" type="text">确定</el-button>
            </el-row>
        </el-popover>
    </div>
</template>
...
export default {
    ...
    props: {
        ...
        obj: {
            ...
            default: () => {
                return {
                    id: 'buildingId',// ID
                    label: 'buildingName',// 显示名称
                    children: 'children', //子级字段名
                    disabled: 'disabled',//设置节点是否可以被选中
                    path: 'path',//路径
                    content: 'content',//描述
                    pid: 'parentId',//父id
                }
    ...
    created () {
        // if (this.ParentNodeIsClick) {
        //     this.disabledParent()
        // } else {
        //     console.log("ParentNodeIsClick===", this.ParentNodeIsClick)
        // }
    },
    methods: {
        init () {
            ...
            if (this.defaultKey != undefined && this.defaultKey.length > 0) {
                if (this.multiple) {
                    ...
                    // 单选
                    if (this.defaultKey[0] != undefined) {
                        if (Object.prototype.toString.call(this.defaultKey).indexOf("Number") != -1
                            ...
                            if (Object.prototype.toString.call(this.defaultKey).indexOf("Array") != -1) {
                                this.setKey(this.defaultKey[0]);
                            } else {
                                console.log("单选：默认节点类型", Object.prototype.toString.call(this.defaultKey));
                ...
                console.log("this.defaultKey===", this.defaultKey)
            }
        },
        //下拉框select点击[入口]
        selectClick () {
        ...
        nodeClick (data, node) {
            if (!this.multiple) {//单选
                console.log("单选:设置、初始化值 key", data, node)
                if (node != null) {
                    if (node.data.treeNodeType == 2) {
        ...
        // disabledParent (data,node) {
        //     console.log(data,node)
        //     // console.log("执行父节点不可选中逻辑")
