...
import { crmCustomerQueryProjectAPI } from '@/api/crm/customer'
// import {
...
export default {
  name: 'RelativeProject', // 相关联系人商机  可能再很多地方展示 放到客户管理目录下（新建时仅和客户进行关联）
  components: {
  ...
  computed: {
    ...
    // 是否能关联
    canRelation() {
      return false;//this.crmType == 'contacts'
    }
  },
  watch: {
    id(val) {
  ...
  methods: {
    ...
    getFieldList() {
      ...
      this.fieldList.push({
        prop: 'status', width: '200', label: '项目状态'
      })
    },
    ...
    getDetail(loading = true) {
      ...
      const request = {
        ...
        customer: crmCustomerQueryProjectAPI
      }[this.crmType]
