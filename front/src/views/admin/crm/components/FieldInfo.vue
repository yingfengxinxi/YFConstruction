<template>
  <div class="field-info-section">
    ...
    <div
      v-if="showQuery"
      class="item-check-section">
      <el-checkbox v-model="field.isQuery" >允许查询</el-checkbox>
    </div>
    <!-- <div
...
export default {
  ...
  computed: {
    ...
    //是否展示查询列v1.0 单行文本、多行文本、下拉、多选、数字、日期、日期时间
    showQuery() {
      if (
        this.field.formType == 'text' ||
        this.field.formType == 'textarea' ||
        this.field.formType == 'select' ||
        this.field.formType == 'checkbox' ||
        this.field.formType == 'number' ||
  ...
  mounted() {
    console.log(this.field.formType)
    if (this.showSelect && this.field.showSetting.length == 0) {
      this.field.showSetting = [
        { value: '选1' },
        { value: '选2' },
