		...
		<div class="content_left">
			<div class="statistics_box">
				<div class="statistics_num_box">
					<div class="subject"><img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    src="../../../../../static/img/Symbol.png">安全隐患数量统计</div>
					<div class="hezi">
						<div class="yh_tj">
							<div class="wcl_yh">
								<div  class="wcl_tit">未处理隐患</div>
								<div class="wcl_num">{{unfinishNumber}}</div>
							</div>
							<div id="untrea"></div>
						</div>
						<div id="yhLX"></div>
				...
				<div class="cl_time">
					<div class="subject"><img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    src="../../../../../static/img/Symbol.png">近30天隐患整改数据</div>
					<div class="cl_time_box">
						<div class="cl_box">
							<img   :onerror='`this.src="${require("@/assets/noimgsrc.png")}"`'    src="../../../../../static/img/cl2.png" />
							<div class="cl_con">
								<div class="cl_top_box">
									<div class="cl_num">{{avgTime}}</div>
									<div class="cl_utils">天</div>
								</div>
								<div class="cl_tit">平均整改时间</div>
							</div>
						</div>
						<div class="cl_box">
							...
							<div class="cl_con">
								<div class="cl_top_box">
									<div class="cl_num">{{quantityCompletion}}</div>
									<div class="cl_utils">次</div>
								</div>
								<div class="cl_tit">隐患解除数量</div>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="content_center">
			<div class="trend_analysis">
				...
				<div class="active_box">
					...
					<div :class="active==3?'hid_active':''" @click="hidActive(3)">按年</div>
				</div>
				<div id='hidden'>
		...
		<div class="content_right">
			<div class="ranking_box">
				...
				<div class="rank_box">
					...
					<div class="rank_con_box">
						<div class="rank_con_con" v-for="item in dangerRankList">
							<div class="rank_con rank_img" style="width: 40%;">
								<div class="kuang"></div> {{item.projectName}}
							</div>
							<div class="rank_con" style="width: 20%;justify-content: center;">{{item.notRectifyCount}}</div>
							<div class="rank_con" style="width: 20%;justify-content: center;">{{item.todayCount}}</div>
							<div class="rank_con" style="width: 20%;justify-content: center;">{{item.total}}</div>
						</div>
					</div>
				</div>
			</div>
			<div class="ranking_box">
				...
				<div class="rank_box">
					...
					<div class="rank_con_box">
						<div class="rank_con_con prescription_con_con" v-for="item in agingAnalysisList">
							<div class="rank_con rank_img" style="width: 40%;">
								<div class="kuang kuang2"></div> {{item.projectName}}
							</div>
							<div class="rank_con" style="width: 30%;justify-content: center;">{{item.percent}}</div>
							<div class="rank_con" style="width: 30%;justify-content: center;">{{item.avgTime}}</div>
						</div>
					</div>
				</div>
			</div>
...
<script>
	...
	import {selectProDangerNumber, selectProDangerLevelProportion, selectProDangerTypeProportion,
			selectProDangerRectifyEfficiency, selectTrendAnalyses, selectProDangerRank, selectDangerNumberProportion,
			selectProAgingAnalysis} from '../api/inspectionStatisticsNew'
	export default {
		data() {
			return {
				active: 1,
		...
		mounted() {
			...
			// 安全隐患趋势分析
			this.getHidden()
			this.initHiddenData(this.active)
			// 安全隐患级别占比
			this.getlevel()
			this.initLevelData()
			// 安全隐患类型占比
			...
			this.getyhLX()
			// 近30天隐患整改数据
			this.initRectifyData()
			// 安全隐患排名
			this.initDangerRankList()
			// 整改时效分析
			this.initAgingAnalysisList()
			this.remsize()
		},
		methods: {
			getyhLX() {
				...
				selectProDangerLevelProportion().then(res => {
					let map = res.data
				
					let data = [{
							name: '一般隐患',
							value: map.generalNumber,
						},
						{
							name: '重大隐患',
							value: map.seriouslNumber,
						},
					];
			...
			getuntrea() {
				...
				myChart.setOption(option)
				this.untreaOption = option
				this.untreaChart = myChart
			},
			initUntreaData() {
				selectProDangerNumber().then(res => {
					let data = res.data
					
					this.unfinishNumber = data.unfinishNumber
					
					let dataList = []
					let percentStr = data.percent.split('%')[0]
					dataList.push(percentStr)
					this.untreaOption.series[0].data = dataList
					this.untreaOption.title.text = data.percent
					this.untreaChart.setOption(this.untreaOption, true)
				}).catch(() => {
				})
			},
			// 近30天隐患整改数据
			initRectifyData() {
				selectProDangerRectifyEfficiency().then(res => {
					this.avgTime = res.data.avgTime
					this.quantityCompletion = res.data.quantityCompletion
				}).catch(() => {
				})
			},
			getType() {
			...
			initTypeData() {
				selectProDangerTypeProportion().then(res => {
					let data = res.data
					let dataList = [
						{
							name: '基础管理类隐患',
							value: data.basiclNumber
						}, {
							name: '生产现场类隐患',
							value: data.productionlNumber
						}
					]
					this.typeOption.series[0].data = dataList
			...
			initLevelData() {
				selectProDangerLevelProportion().then(res => {
					...
					let dataList = [
						{
							name: '一般事故隐患',
							value: data.generalNumber
						}, {
							name: '重大事故隐患',
							value: data.seriouslNumber
						}
					]
					this.levelOption.series[0].data = dataList
					this.levelChart.setOption(this.levelOption, true)
				}).catch(() => {
				})
			},
			hidActive(num) {
            ...
            //symbol: 'none',
				...
				myChart.setOption(option)
				this.hiddenOption = option
				this.hiddenChart = myChart
			},
			initHiddenData(type) {
				selectTrendAnalyses(type).then(res => {
					let resultData = res.data
					let xData = []
					let yData = []
					
					resultData.forEach(item => {
						xData.push(item.formatDate)
						yData.push(item.number)
					})
					
					this.hiddenOption.xAxis.data = xData
					this.hiddenOption.series[0].data = yData
					this.hiddenOption.series[1].data = yData
					
					this.hiddenChart.setOption(this.hiddenOption, true)
				})
			},
			getDanger() {
				...
				myChart.setOption(option)
				this.dangerOption = option
				this.dangerChart = myChart
			},
			initDangerData() {
				selectDangerNumberProportion().then(res => {
					let resultData = res.data
					let xData = []
					let yData1 = []
					let yData2 = []
					...
					resultData.forEach(item => {
						xData.push(item.formatDate)
						yData1.push(item.notRectify)
						yData2.push(item.notRecheck)
						yData3.push(item.complete)
					})
					
					this.dangerOption.xAxis.data = xData
					this.dangerOption.series[0].data = yData1
					this.dangerOption.series[1].data = yData2
					this.dangerOption.series[2].data = yData3
					
					this.dangerChart.setOption(this.dangerOption, true)
					
				})
			},
			initDangerRankList() {
				selectProDangerRank().then(res => {
					this.dangerRankList = res.data
				}).catch(() => {
				})
			},
			initAgingAnalysisList() {
				selectProAgingAnalysis().then(res => {
					this.agingAnalysisList = res.data
				}).catch(() => {
				})
			},
			remsize() {
