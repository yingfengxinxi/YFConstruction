			...
			if (data.evalApplyItemsList) {
				...
				if (this.action.type == "update") {
					this.formData.list.forEach(item => {
						...
						// 设置为undefined  el-input-number 不会有默认值
						if (item.itemScore == null || item.itemScore == '') {
							item.itemScore = undefined
						}
					})
				} else {
					this.formData.list.forEach(item => {
        ...
        this.$refs['form', 'formData'].validate(valid => {
          if (valid) {
			...
			for (let i = 0; i<this.formData.list.length;i++) {
				...
				if (item.itemScore == undefined) {
					let classTypeName = this.selectDictLabel(this.dictData.classTypeDict, item.classType)
					let errorMsg = classTypeName + "中存在未打分数据，请完成所有打分项！"
					this.$message.error(
					  errorMsg
					)
					return;
